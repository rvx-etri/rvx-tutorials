...
NpxTensorInfo *npx_gen_spike_lfsr(const NpxTensorInfo *input_tensor, uint8_t *plfsr)
{
  ...
  if (spike_tensor == 0)
  {
    ...
    npx_tensor_set_size(spike_tensor, 0, npx_tensor_get_size(input_tensor, 0));
    npx_tensor_set_size(spike_tensor, 1, npx_tensor_get_size(input_tensor, 1));
    npx_tensor_set_size(spike_tensor, 2, npx_tensor_get_size(input_tensor, 2));
  ...
  for (c = 0; c < npx_tensor_get_size(input_tensor, 2); c++)
  {
    int c_offset = c * npx_tensor_get_size(input_tensor, 1) * npx_tensor_get_size(input_tensor, 0);
    for (h = 0; h < npx_tensor_get_size(input_tensor, 1); h++)
    {
      int h_offset = h * npx_tensor_get_size(input_tensor, 0);
      for (w = 0; w < npx_tensor_get_size(input_tensor, 0); w++)
      {
